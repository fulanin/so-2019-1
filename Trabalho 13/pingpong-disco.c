...
void moverBody (void * arg)
{
  ...
  for (i = 0 ; i < blocksPerTask; i++)
  {
    ...
    printf ("T%02d will write block UNO %3d\n", task_id(), block_dest) ;
    if (disk_block_write (block_dest, buffer1) == 0)
      printf ("T%02d wrote block UNO %3d\n", task_id(), block_dest) ;
    else
